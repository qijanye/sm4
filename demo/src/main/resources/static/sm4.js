...
function SM4Util() {
    ...
    this.encryptDataECB=function(plainText){
        try{ 
            var sm4 = new SM4(); 
    ...
    this.decryptDataECB=function(plainText){
    	try{
	    	var sm4 = new SM4();
	        var ctx = new SM4_Context();
	        ctx.isPadding = true;                     
	        ...
	        var decrypted = sm4.sm4_crypt_ecb(ctx, base64js.toByteArray(plainText));
	        return byteToString(decrypted).replace(/(\s*$)/g, "");
    	}catch (e){    
            console.error(e);
            return null;  
        } 
    }
    this.encryptDataCBC=function(plainText){  
         try{  
                var sm4 = new SM4(); 
                var ctx = new SM4_Context();  
                ctx.isPadding = true;  
